...
int main(void)
{
	...
	int menu_modulo, arreglo_clientes[FILAS] = {0};
	do{
	...
	do{
		...
		switch(menu){
			case 1:
					...
					for(k=0;k<cantidad_productos;k++){
						puts("digite los numeros del cliente: ");
						scanf("%d", &clientes_numero[i]);
					}
				   while(getchar()!= '\n');	
			...
			case 2:
					...
					for(i=0;ordenar_alfabeticamente[i][j]!='\0';i++){
						ordenar_alfabeticamente[i][j] = tolower(ordenar_alfabeticamente[i][j]);
						printf("%s\n",ordenar_alfabeticamente[i]);
					}
			break;
			case 3:
				   puts("*** PROMOCION DE SORTEO ***");
				   srand(time(NULL));
				   ...
				   char fechaText[30];
				   strftime(fechaText,50," PROMO SORTEO INVIERNO %A, %d de %B del anio %Y\n", la_hora);
				   puts(fechaText);
				   for(i=0;i<cantidad_productos;i++){
					printf("El numero ganador es %d con el numero %d, en esta fecha y hora, son las %02d:%02d: hs "
					,clientes_numero[i], aleatorio, la_hora->tm_hour,la_hora->tm_min);
				   break;
				   }
			break;
			case 4:
